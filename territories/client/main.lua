...
CheckLocation = function(closest)
  ...
  if closest then
    ...
    if area.openzone or (area.control == PlayerData.job.name or (PlayerData.job2 and PlayerData.job2.name == area.control)) then
      if area.actions and area.actions.entry then 
        if vDist(plyPos,area.actions.entry.location) < Config.InteractDist then
          DrawText3Ds(area.actions.entry.location[1],area.actions.entry.location[2],area.actions.entry.location[3],area.actions.entry.helpText)
    ...
    if not InsideInterior then
      if vDist(plyPos,area.actions.exit.location) < Config.InteractDist then
        DrawText3Ds(area.actions.exit.location.x,area.actions.exit.location.y,area.actions.exit.location.z,area.actions.exit.helpText)
        if GetKeyUp(Config.InteractControl) then
          InsideInterior = area
    ...
    if not InsideInterior then
      ...
      if exitDist and exitDist < Config.InteractDist then
        DrawText3Ds(Territories[closestExit].actions.exit.location[1],Territories[closestExit].actions.exit.location[2],Territories[closestExit].actions.exit.location[3],Territories[closestExit].actions.exit.helpText)
        if GetKeyUp(Config.InteractControl) then
  ...
  if InsideInterior then    
    ...
    if exitDist and exitDist < Config.InteractDist then
      DrawText3Ds(InsideInterior.actions.exit.location.x,InsideInterior.actions.exit.location.y,InsideInterior.actions.exit.location.z,InsideInterior.actions.exit.helpText)
      if GetKeyUp(Config.InteractControl) then
        mLibs:TeleportPlayer(InsideInterior.actions.entry.location)
        InsideInterior = nil          
      end
      ...
      if actDist < Config.InteractDist then
        DrawText3Ds(InsideInterior.actions[closestAct].location.x,InsideInterior.actions[closestAct].location.y,InsideInterior.actions[closestAct].location.z,InsideInterior.actions[closestAct].helpText)
    ...
    if not curSelling then
      ...
      if not soldPeds[curSelling] then
        ...
        if sellItem then
          ...
          progBar:taskBar(Config.SellDrugsTimer,_U["selling_drugs"])
